...
2. **Random pool** — If nothing is available for that section (e.g. sparse DB, dev-light, all unseen exhausted), fetch a shuffled slice of **all** unexpired articles (still excluding `seenArticleIds` when possible).
3. **Resurface** — If the pool is still empty, shuffle among unexpired articles **without** the seen filter so the feed never dead-ends when rows exist.
